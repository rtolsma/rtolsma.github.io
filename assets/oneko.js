...
(function oneko() {
  ...
  const persistPositionFromData = curScript && curScript.dataset.persistPosition;
  ...
  // Try to position next to profile pic on initial load
  function getInitialPosition() {
    const profilePic = document.querySelector('.profile-pic');
    if (profilePic) {
      const rect = profilePic.getBoundingClientRect();
      // Position below the profile pic, centered horizontally
      return {
  ...
  function init() {
    let nekoFile = nekoFileFromData || "./oneko.gif";
    if (persistPositionFromData !== undefined) {
      if (persistPositionFromData === "") {
        persistPosition = true;
      } else {
        persistPosition = JSON.parse(persistPositionFromData.toLowerCase());
      }
    }
    ...
    // If no stored position, start next to profile pic
    if (!hasStoredPosition) {
      const initialPos = getInitialPosition();
      nekoPosX = initialPos.x;
      nekoPosY = initialPos.y;
      mousePosX = initialPos.x;
      mousePosY = initialPos.y;
    }
  ...
  // Wait for page to fully load and layout to stabilize
  function startNeko() {
    // Delay to ensure layout is stable after fonts/images load
    setTimeout(init, 300);
  }
  ...
  if (document.readyState === 'complete') {
    ...
    window.addEventListener('load', startNeko);
  }
})();
